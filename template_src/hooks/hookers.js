    ...
    startWebpackDevServer() {
      ...
      function fixedArgs(){
        ...
        for (let i = 0; i < process.argv.length; i++) {
          ...
          if (wpArgsEnvActive) {
            if(process.argv[i].indexOf("--") == 0 ){
              wpArgs.push(process.argv[i].replace("--", "--env."));
            }
            else{
              wpArgs.push(process.argv[i]);
            }
          ...
          if (process.argv[i] == "--env") {
            wpArgsEnvActive = true;
          }

          if (process.argv[i].indexOf("--wp.") == 0) {
          	wpArgsActive = true;
            wpArgs.push(process.argv[i].replace("--wp.", "--"));
          }
        }

				if(wpArgs.length > 0){
					args = args.concat(wpArgs);
				}
      }
